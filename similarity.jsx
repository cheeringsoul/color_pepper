// ===== Similarity page =====
function SimilarityPage({ data, onOpenSymbol }) {
  const sim = data.similarity;
  const [algo, setAlgo] = React.useState('pearson');
  const [tf, setTf] = React.useState('4h');
  const [period, setPeriod] = React.useState('30d');
  const [minCorr, setMinCorr] = React.useState(50);
  const [refSym, setRefSym] = React.useState('BTC');
  const [active, setActive] = React.useState(new Set(['ETH','BNB','SOL','AVAX']));
  const [hidden, setHidden] = React.useState(new Set());

  const colors = ['#627eea','#f3ba2f','#9945ff','#e84142','#00ec97','#2a5ada','#0088cc','#28a0f0'];
  const palette = {};
  sim.candidates.forEach((c, i) => palette[c.sym] = colors[i % colors.length]);

  const filtered = sim.candidates.filter(c => c.corr * 100 >= minCorr);
  ...
  const allSeries = [{ sym: refSym, color: 'var(--accent)', norm: sim.ref.norm, ref: true }];
  sim.candidates.filter(c => active.has(c.sym) && !hidden.has(c.sym)).forEach(c => {
    allSeries.push({ sym: c.sym, color: palette[c.sym], norm: c.norm, corr: c.corr });
  });
  const allVals = allSeries.flatMap(s => s.norm);
    ...
    <div className="page sim-page">
      ...
      <div className="card sim-overlay-card">
        ...
        <div className="sim-legend">
          ...
          {sim.candidates.filter(c => active.has(c.sym)).map(c => (
      ...
      <div className="card sim-candidates">
        ...
        <div className="candidates-grid">
          {filtered.map(c => {
            const sym = data.SYMBOLS.find(s => s.sym === c.sym);
            return (
...
window.SimilarityPage = SimilarityPage;
