// ===== Agent page =====
function AgentPage({ data, contextSym, onOpenSymbol }) {
  ...
  function send() {
    ...
    const next = [...messages, { role: 'user', body: input }];
    next.push({
      role: 'ag',
      body: <p>正在分析 <strong>{contextSym || 'BTC'}</strong> 的相关数据... <span className="dim">(demo)</span></p>,
    });
    setMessages(next);
    setInput('');
  }
